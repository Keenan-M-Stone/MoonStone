...
def test_kerr_gpu_vs_cpu():
    src = (10.0, 0.0, 0.0)
    dirs = np.array([[-1.0, -0.05, 0.0]], dtype=float)
    sources = np.array([[10.0,0.0,0.0]], dtype=float)
    masses = np.array([1.0], dtype=float)
    params = {'npoints': 64, 'step': 1e-3, 'spin': (0.0,0.0,0.4)}
    gpu_res = ac.trace_kerr_gpu(sources, dirs, masses, params)
    cpu_res = geodesics.trace_kerr_formal(src, (-1.0, -0.05, 0.0), mass=1.0, spin=(0.0,0.0,0.4), params={'npoints':64, 'step':1e-3, 'formal': True})
    assert len(gpu_res) == 1
    gr = gpu_res[0]
    assert len(gr) == len(cpu_res)
    # compare final point tolerance
    dx = abs(gr[-1][0] - cpu_res[-1][0])
    dy = abs(gr[-1][1] - cpu_res[-1][1])
    dz = abs(gr[-1][2] - cpu_res[-1][2])
    assert (dx*dx + dy*dy + dz*dz)**0.5 < 1e-1
